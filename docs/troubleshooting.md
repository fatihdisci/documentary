...
### Subtitles run slightly ahead of the voice

Cues are placed on the real word boundaries the TTS engine reports, and those
are stored beside the audio. Narration generated by an older version has no
stored timings, so its subtitles are placed by estimate instead — which reads as
them arriving a fraction of a second early, growing to about half a second
inside a long scene.

The render warns when this applies. To fix it, open the **Audio** tab and
regenerate narration once; the new take stores its word timings and every later
render (and any Short cut from it) is timed to the words. Imported audio has no
word boundaries to work from and always uses the estimator, so its cues stay
approximate — the leading silence is measured and excluded, but the timing
inside a clip is still inferred from the text.

Note that subtitles are **burned into** the finished video, so a Short inherits
whatever the long render had. Re-render the long video first, then rebuild the
Short from the new export.
